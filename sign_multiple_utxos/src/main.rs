...
fn main() {
    ...
    let tx = sign(secp, spending_transaction, &script_pubkey, &privkey.private_key, pubkey.public_key.key);
...
fn sign(secp: Secp256k1<secp256k1::All>, mut tx: Transaction, script_pubkey: &Script, privkey: &PrivateKey, pubkey: PublicKey) -> Transaction {
    ...
    for (ix, input) in tx.input.iter_mut().enumerate(){
        ...
        let serialized_signature = secp.sign(&Message::from_slice(&sighash[..]).unwrap(), &privkey.key).serialize_der();

        let mut with_hashtype = serialized_signature.to_vec();
        with_hashtype.push(SigHashType::All.as_u32() as u8);
        input.witness.clear();
        println!("with hash: {:?}", privkey.key);
        let script_sig = Builder::new()
                            .push_slice(with_hashtype.as_slice())
                            .push_slice(serialize_pubkey(pubkey).as_slice())
                            .into_script();
